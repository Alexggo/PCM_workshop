...
# Leave some space to the left
plot(NA,xlim=c(0,693),ylim=c(1,Ntip(vertebrate.tree)),
     bty="n",axes=FALSE,xlab="",ylab="")
#points(400,5)
grid()
axis(side=3)
axis(side=4,at=1:Ntip(vertebrate.tree))

plotTree(vertebrate.tree,ftype="i",
         fsize=0.7,xlim=c(0,693),ylim=c(1,Ntip(vertebrate.tree)),
         add=TRUE,mar=par()$mar)
abline(v=473,lty="dotted",col="blue")

par(mar=c(2.1,1.1,1.1,1.1))
plotTree(vertebrate.tree,ftype="i",
         fsize=0.7,xlim=c(-220,693),ylim=c(1,Ntip(vertebrate.tree)),
         mar=par()$mar,direction="leftwards")

axis(1)

plotTree(vertebrate.tree,ftype="i",
         fsize=0.7,xlim=c(693,-220),ylim=c(1,Ntip(vertebrate.tree)),
         mar=par()$mar,direction="leftwards")
axis(1,at=seq(0,500,by=50),cex.axis=0.6)


plotTree(vertebrate.tree,ftype="i",
         fsize=0.7,xlim=c(693,-220),ylim=c(Ntip(vertebrate.tree),1),
         mar=par()$mar,direction="leftwards")
grid()
axis(1,at=seq(0,500,by=50),cex.axis=0.6)


cols <- setNames(c(rgb(153,192,141,max=255),
...
# xx and xy are the coordinates of all nodes and tips.
...
# Coordinates in the order of the index
pp$xx

robin_x <- mean(c(pp$xx[tip_robin],pp$xx[parent_robin]))
#abline(v=robin_x)

robin_y <- pp$yy[tip_robin]

lines(x=rep(robin_x,2),
      y=robin_y+c(-0.25,0.25),lwd=11)
lines(x=rep(robin_x,2),
      y=robin_y+c(-0.25,0.25),lwd=7,col="lightblue")


data("liolaemid.tree")
data("liolaemid.data")

plotTree(liolaemid.tree,ftype="off",
...
for (i in 1:Ntip(liolaemid.tree)){
  x <- c(pp$xx[i]-0.5,pp$xx[i]+0.5,
         pp$xx[i]+0.5,pp$xx[i]-0.5)
  y <- pp$yy[i]+c(0,0,h,h)
  color <- cols[parity_mode[liolaemid.tree$tip.label[i]]]
...
dev.off()
# Plot a phylogeny from scratch:
plotTree(vertebrate.tree,plot=FALSE)
pp <- get("last_plot.phylo",envir=.PlotPhyloEnv)
points(pp$xx,pp$yy)

#type="arc",arc_height=0.5
...
slantedTree <- function(phy){
  ...
  for(i in 1:nrow(phy$edge)){
    lines(pp$xx[phy$edge[i,]],
          pp$yy[phy$edge[i,]],col="blue",lwd=3)
  }
  for (i in 1:Ntip(phy)){
