...
gd<-read.csv("grunts.csv", row.names=1,
...
hd<-data.frame(Genus.species=rownames(gd),
               x=gd[,"habitat"])
head(hd)
# make a transition matrix with one hidden char
rates.hisse<-TransMatMakerHiSSE(hidden.traits=1)
rates.hisse
# we also want to make the bisse model but 
# in hisse
rates.bisse<-TransMatMakerHiSSE(hidden.traits=0)
rates.bisse

# fit bisse model, but using hisse
...
# fit cid (character independent diversification)
# this is the bisse null
...
hAIC<-setNames(c(cid.mle$AICc, bisse.hmle$AICc, 
           hisse.mle$AICc), c("cid null", "bisse",
                              "hisse"))
aicw(hAIC)
